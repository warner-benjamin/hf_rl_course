...
def train(args, parse=False):
    ...
    agent = A2C(envs.single_action_space.shape[0], envs.single_observation_space.shape[0]).to(device)
    ...
    actions = torch.zeros((args.num_steps, args.num_envs) + envs.single_action_space.shape, dtype=torch.float32)
    rewards = torch.zeros((args.num_steps, args.num_envs), dtype=torch.float32)
    dones = torch.zeros((args.num_steps, args.num_envs), dtype=torch.float32)
    values = torch.zeros((args.num_steps, args.num_envs), dtype=torch.float32)
    ...
    next_obs = torch.from_numpy(envs.reset()).to(dtype=torch.float32)
    next_done = torch.zeros(args.num_envs, dtype=torch.float32)
    ...
    for global_step in range(0, args.total_timesteps+args.num_envs, args.num_envs):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad(), ac:
                action, _, _, value = agent(next_obs.to(device=device))
            values[step] = value.flatten().cpu()
            actions[step] = action.cpu()

            next_obs, reward, next_done, infos = envs.step(actions[step].numpy())

            next_obs = torch.from_numpy(next_obs).to(dtype=torch.float32)
            rewards[step] = torch.from_numpy(reward).to(dtype=torch.float32)
            next_done = torch.from_numpy(next_done)
        ...
        with torch.no_grad(), ac:
            next_value = agent.get_value(next_obs.to(device=device)).reshape(1, -1)
        ...
        # flatten the batch
        b_obs = obs.reshape((-1,) + envs.single_observation_space.shape).to(device=device, non_blocking=True)
        b_actions = actions.reshape((-1,) + envs.single_action_space.shape).to(device=device, non_blocking=True)
        b_advantages = advantages.reshape(-1).to(device=device, non_blocking=True)
        b_returns = returns.reshape(-1).to(device=device, non_blocking=True)
